...
class model_6_v1(nn.Module):
    ...
    def score_sample(self, z, recons_loss):
        # recons_loss = recons_loss.to(self.device)
        # recons_loss = recons_loss.squeeze(1)
    ...
    def forward(
        ...
        if self.mode == 'train':
            ...
            if sample_type == 'neg':
                ...
                if self.turn_on_noise:
                    z = z + r_noise

            sample_score = self.score_sample(z, loss_md)
            return z, sample_loss, sample_score
        ...
        # ================================================ #
            ...
            s = self.score_sample(z, sample_loss)
            return s
...
class model_6_v1_container():
    def __init__(
        ...
        self.network_module = model_6_v1(
            device,
            latent_dim,
            encoder_structure_config,
            decoder_structure_config,
    ...
    def train_model(
        ...
        for epoch in tqdm(range(1, self.num_epochs + 1)):
            ...
            for b in range(num_batches):
                ...
                # Positive sample
                z_p, batch_loss_pos, sample_score_pos = self.network_module(_x_p, sample_type='pos')
                batch_loss_neg = []
                sample_scores_neg = []
                ...
                for ns in x_neg:
                    ns = ns.squeeze(1)
                    _z_n, n_sample_loss, n_sample_score = self.network_module(
                        ns,
                        sample_type='neg'
                    )

                    # Sample loss has shape [ batch, num_fields ]
                    batch_loss_neg.append(
                        torch.mean(n_sample_loss, dim=1, keepdims=True)
                    )
                    sample_scores_neg.append(n_sample_score)
                    sim_val = F.cosine_similarity(z_p,_z_n)
                    emb_similarity_loss.append(sim_val)

                # Shape : [ batch, num_neg_samples ]

                batch_loss_neg = torch.cat(batch_loss_neg, dim=1)
                sample_scores_neg = torch.cat(sample_scores_neg, dim=1)
                sample_scores_neg = sample_scores_neg.squeeze(1)

                # ========================
                emb_similarity_loss = torch.stack(emb_similarity_loss, dim=-1)
                emb_similarity_loss = torch.mean(emb_similarity_loss,dim=1, keepdim=False)
                emb_similarity_loss = torch.mean(emb_similarity_loss,dim=0, keepdim=False)
                ...
                if epoch_phase == 3:
                    b_epoch_losses_phase_3.append(score_loss)
                # ====================
                # Clip Gradient
                # ====================
            ...
            # Ensure convergence
            # ==========================
            if len(b_epoch_losses_phase_3) > 0 :
                epoch_losses_phase_3.append(np.mean(loss_value))

            print('Epoch loss ::', np.mean(epoch_losses))

        self.network_module.mode = 'test'
        return epoch_losses, epoch_losses_phase_3
